...
public class DriveStraightController extends TimerTask{
    ...
    public DriveStraightController(Segment[] leftTrajectory, Segment[] rightTrajectory, double leftMaxV, double rightMaxV, double controlLoop){
    ...
    public void run(){
        if(!rightSide.isTrajDone() && !leftSide.isTrajDone()){
            System.out.println("Right Side Motor Val: "+this.rightSide.calcMotorOutput(this.rightCurrPosition)+"----- Left Side Motor Val: "+this.leftSide.calcMotorOutput(this.leftCurrPosition)+"\n");
        }else{
            System.exit(0);
        }
    }    
